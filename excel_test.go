...
func itoa(i int) string { // small helper without importing strconv
...
func TestStaticHeader_Valid(t *testing.T) {
	...
	if err := StaticHeader(path, "Sheet1"); err != nil {
...
func TestStaticHeader_InvalidInputs(t *testing.T) {
	if err := StaticHeader("", "X"); err == nil {
		t.Fatal("expected error for empty path")
	}
	path := t.TempDir() // path is a directory
	if err := StaticHeader(path, "X"); err == nil {
		t.Fatal("expected error for directory path")
	}
	path = createTempWorkbook(t, "S", [][]interface{}{{"A"}})
	if err := StaticHeader(path, "Missing"); err == nil {
...
func TestEnableData_AppliesFilter(t *testing.T) {
	...
	if err := EnableData(path, "Data"); err != nil {
...
func TestAutoSize_SetsReasonableWidths(t *testing.T) {
	...
	if err := AutoSize(path, "Sheet1"); err != nil {
	...
	if wA < 10.0 {
		t.Fatalf("expected column A width >= 10, got %f", wA)
	}
	if wB <= wA {
...
func TestAccent1_AppliesStyleToHeader(t *testing.T) {
	...
	if err := Accent1(path, "Styled"); err != nil {
	...
	if err != nil {
		t.Fatalf("GetCellStyle B1: %v", err)
	}
	sC, err := f.GetCellStyle("Styled", "C1")
	if err != nil {
		t.Fatalf("GetCellStyle C1: %v", err)
	}
...
func TestAccent1_EmptyHeaderErrors(t *testing.T) {
	path := createTempWorkbook(t, "Empty", [][]interface{}{})
	if err := Accent1(path, "Empty"); err == nil {
...
func TestSecureHandling_InvalidPath(t *testing.T) {
	...
	for _, fn := range []func(string, string) error{StaticHeader, EnableData, AutoSize, Accent1} {
		if err := fn(tmp, "Sheet1"); err == nil {
			t.Fatalf("expected error for missing path with %p", fn)
		}
	}
}
...
func TestCSVToExcel_CommaSeparator(t *testing.T) {
	...
	if err := CSVToExcel(csvPath, excelPath, "Sheet1", ','); err != nil {
	...
	// Ensure formula injection is neutralized
	b2, err := f.GetCellValue("Sheet1", "B2")
	if err != nil {
		t.Fatalf("get B2: %v", err)
	}
	if b2 != "=2+2" {
		t.Fatalf("expected literal '=2+2' in B2, got %q", b2)
	}
}
...
func TestCSVToExcel_SemicolonSeparator(t *testing.T) {
	...
	if a1 != "A" || c2 != "3" {
		t.Fatalf("unexpected values: A1=%q C2=%q", a1, c2)
	}
}
